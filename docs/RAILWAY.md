...
| Pre-deploy command failed | Open deploy logs → **Pre deploy** tab. Usually `DATABASE_URL` is missing on the API service, or Postgres SSL — ensure `DATABASE_URL=${{Postgres.DATABASE_URL}}` is set. |
| `column users.role does not exist` | Redeploy after pulling latest `main` — Alembic adds RBAC columns in pre-deploy. Or run `cd apps/api && python scripts/migrate.py` against production `DATABASE_URL`. |
